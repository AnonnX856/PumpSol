...
const PoolCard: React.FC<{ poolId: string; isExpanded?: boolean; onToggle?: () => void }> = ({ 
  poolId, 
  isExpanded = false, 
  onToggle 
}) => {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'migrated': return 'bg-blue-100 text-blue-800';
    ...
    <div className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-200">
      ...
      <div className="p-6 border-b border-gray-200">
        <div className="flex justify-between items-start mb-4">
          <div>
            <h3 className="text-xl font-bold text-gray-900">{pool.tokenSymbol}</h3>
            <p className="text-sm text-gray-600">{pool.tokenName}</p>
            <p className="text-xs text-gray-500 font-mono">{pool.poolId.slice(0, 16)}...</p>
          </div>
          <div className="flex flex-col items-end gap-2">
            <span className={`px-3 py-1 rounded-full text-xs font-medium ${getStatusColor(pool.migrationStatus)}`}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
          <div>
            ...
            <p className="text-lg font-semibold text-gray-900">${pool.tokenPrice.toFixed(8)}</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="text-lg font-semibold text-gray-900">{pool.migrationProgress.toFixed(1)}%</p>
          </div>
        </div>
        ...
        <div className="mb-4">
          <div className="flex justify-between items-center mb-1">
            ...
            <span className="text-sm text-gray-500">{pool.migrationProgress.toFixed(1)}%</span>
          </div>
          <div className="w-full bg-gray-200 rounded-full h-3">
            <div
              className={`h-3 rounded-full transition-all duration-300 ${getProgressColor(pool.migrationProgress)}`}
              style={{ width: `${Math.min(pool.migrationProgress, 100)}%` }}
            ></div>
          </div>
          {pool.migrationProgress >= 100 && (
            <p className="text-xs text-blue-600 mt-1">🎉 Pool has migrated to AMM!</p>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            ...
            <span className="font-medium ml-2">{pool.totalTokensSold.toLocaleString()}</span>
          </div>
          <div>
            <span className="text-gray-500">Total Supply:</span>
            <span className="font-medium ml-2">{pool.totalSupply.toLocaleString()}</span>
          </div>
        </div>
      </div>
      ...
      {isExpanded && (
        <div className="p-6 space-y-6">
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            <h4 className="font-semibold text-gray-900 mb-4">Trade Calculator</h4>
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-semibold text-blue-900">
                    {tradeEstimate.output.toFixed(6)} {tradeEstimate.outputToken}
                  </span>
                </div>
                <div className="flex justify-between items-center mt-1">
                  <span className="text-xs text-gray-500">Price Impact:</span>
                  <span className="text-xs text-orange-600">~{tradeEstimate.priceImpact}%</span>
                </div>
              </div>
            )}
            ...
            <p className="text-xs text-gray-500 mt-3">
              ⚠️ Estimates only. Actual prices may vary due to bonding curve mechanics and slippage.
            </p>
          </div>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-mono text-xs">{pool.tokenMint.slice(0, 16)}...</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">${pool.tokenPrice.toFixed(8)}</span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{pool.currentFunding.toFixed(2)} SOL</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{(pool.fundingGoal - pool.currentFunding).toFixed(2)} SOL</span>
                </div>
              </div>
            </div>
          </div>
    ...
    return (
      <div className="min-h-screen bg-gray-100 flex items-center justify-center">
        <div className="bg-red-50 border border-red-200 rounded-lg p-6 max-w-md">
          <h2 className="text-red-800 font-semibold mb-2">Error Loading Pools</h2>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {filteredPools.length === 0 ? (
          <div className="bg-white rounded-lg shadow-sm p-12 text-center">
            ...
            <p className="text-gray-500">No pools match your current filter criteria.</p>
          </div>
        ) : (
